...
labelsh = ['synset', 'depth', 'p.', 'i.', 'h', 'peaks', 'total']
labels = []
data = []
for layer in adict6:
    for entry in adict6[layer]:
        ...
        data.append([layer, *data_[1], *data_[2]])
...
DL(fname_[:-4]+"_",[1],[1,5],[2,4,5,6,7,8,10,11,12,13,14,15])
...
ME(fname_[:-4],"\\bf",[(0,i) for i in range(0,7)])
DL=g.tableHelpers.dl
DL(fname_[:-4]+"_",[1],[1,5],[2,3,4,5,6,8,9,10,11,13,14,15,17,18])
